...
class SshTerminalBridge(
    ...
    private fun startReading() {
        scope.launch(Dispatchers.IO) {
            ...
            try {
                while (isActive) {
                    // SSHJ (≤0.38.0) 在处理较大终端窗口（平板尺寸）时，内部通道
                    // 数据处理链存在无限递归缺陷，会耗尽线程栈触发 StackOverflowError。
                    // 此处捕获后主动断开，由上层自动重连逻辑处理，防止应用崩溃。
                    val n = try {
                        session.inputStream.read(buffer)
                    } catch (e: StackOverflowError) {
                        Log.e("SshTerminalBridge", "SSHJ stack overflow in stream read — disconnecting", e)
                        break
                    }
                    if (n == -1) break

...
class TerminalViewModel @Inject constructor(
    ...
    fun onTerminalResize(cols: Int, rows: Int) {
        ...
        resizeJob = viewModelScope.launch {
            // 平板等大屏设备布局完成很快，会在 SSHJ 完成握手前立即触发 PTY 调整。
            // 若当前仍是默认尺寸（80×24），说明这是首次 resize，给 SSHJ 多一点时间
            // 完成连接协商，避免在握手期间发送窗口大小变更而触发递归缺陷。
            val delayMs = if (termCols == 80 && termRows == 24) 600L else 300L
            kotlinx.coroutines.delay(delayMs)

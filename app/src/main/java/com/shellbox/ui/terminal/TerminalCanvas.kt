...
private fun resolveColor(encodedColor: Int, colors: TerminalColors): Int =
 ...
 * and appearance-driven colors / cursor / spacing.
 */
@Composable
fun TerminalCanvas(
    ...
    val scheme = remember(appearance, systemDark) {
        appearance.resolvedScheme(systemDark)
    }
    ...
    Box(modifier = modifier) {
        Canvas(
            ...
            // Fill background from scheme
            drawRect(Color(defaultBg), Offset.Zero, Size(size.width, size.height))
            ...
            for (row in 0 until rows) {
                ...
                val showCursorCell = effectiveScroll == 0 && row == cursorRow
                ...
                while (col < cols && charIndex < line.spaceUsed) {
                    ...
                    var fg = resolveColor(fgIdx, colors)
                    var bg = resolveColor(bgIdx, colors)
                    // Map default indices to scheme colors
                    ...
                    // Draw cursor on top of cell
                    if (showCursorCell && col == cursorCol && cursorVisible) {
                        val cc = Color(cursorColor)
                        when (appearance.cursorStyle) {
                            CursorStyle.BLOCK -> {
                                drawRect(cc.copy(alpha = 0.85f), Offset(cellX, cellY), Size(cellW, cellH))
                            }
                            CursorStyle.UNDERLINE -> {
                                drawRect(cc, Offset(cellX, cellY + cellH - 2.5f), Size(cellW, 2.5f))
                            }
                            CursorStyle.BAR -> {
                                drawRect(cc, Offset(cellX, cellY), Size(2.5f, cellH))
                            }
                        }
                    }
                ...
                // Empty cursor cell at end of short line
                if (showCursorCell && cursorVisible && col <= cursorCol && cursorCol < cols) {
                    val cellX = cursorCol * cellW
                    val cellY = row * cellH
                    val cc = Color(cursorColor)
                    when (appearance.cursorStyle) {
                        CursorStyle.BLOCK ->

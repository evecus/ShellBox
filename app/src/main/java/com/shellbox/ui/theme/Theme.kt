...
fun ShellBoxTheme(content: @Composable () -> Unit) {
    ...
    if (!view.isInEditMode) {
        SideEffect {
            ...
            // Do NOT set window.statusBarColor here: it's deprecated under edge-to-edge
            // (Activity.enableEdgeToEdge()) and on some OEM tablet builds it fights with
            // the edge-to-edge decor, causing content to be drawn either under an opaque
            // bar or with insets misreported as zero — i.e. the app content overlapping
            // the status bar. Edge-to-edge + WindowInsets (consumed per-screen via
            // TopAppBar/statusBars insets) is the only status-bar styling needed; we only
            // control icon contrast here.
